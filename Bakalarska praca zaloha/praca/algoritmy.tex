...
V nasledujúcom uvedieme niektoré vylepšenia doposiaľ popísaných algoritmov. Tie ich budú schopné zrýchliť, pričom ale budú uvažovať nejaké obmedzenia, prípadne nebudú garantovať nájdenie najoptimálnejšieho riešenia.


\subsubsection{Algoritmus A*}

Vyhľadávací algoritmus A* je založený na princípe Dijkstrovho algoritmu. Prišiel s ním Nils Nilsson v roku 1964 a v roku 1967 ho Peter E. Hart vylepšil a o rok neskôr dokázal Bertram Raphael jeho optimálnosť, pričom zaviedol i pomenovanie A* (alebo aj $A star$).\newline
...
Ako sme už spomínali, algoritmus sa veľmi podobá Dijkstrovmu. Rozdiel je ten, že pri vyberaní nasledujúceho vrcholu pre nadchádzajúcu iteráciu algoritmu berieme vrchol s najnižšou hodnotou $f(v)$, kde $v$ sú všetky relevantné vrcholy pre ďalšiu iteráciu. Funkcia $f(x) = g(x) + h(x)$ pre nejaký vrchol $x$, pričom funkcia $g(x)$ predstavuje cenu nájdenej najlacnejšej cesty od začiatočného vrcholu do vrcholu $x$ a funkcia $h(x)$ odhaduje cenu najlacnejšej cesty z vrcholu $x$ do koncového vrcholu. Funkcia $h(x)$ teda predstavuje spomínanú heuristickú funkciu. Navyše, táto heuristická funkcia nemôže nadhodnotiť cenu najlacnejšej cesty k cielu, inak by algoritmus mohol prehliadnuť najoptimálnejšie riešenie. Táto vlastnosť sa nazýva $prípustnosť$. Na porovnanie, ak uvažujeme graf, kde sú všetky hrany ohodnotené rovnako, Dijkstrov algoritmus bude prehľadávať graf do všetkých strán rovnomerne, zatiaľ čo A* algoritmus bude, vďaka heuristike, vyberať hrany, ktoré dospejú skorej ku koncovému vrcholu. Ako je vyobrazené na~\ref{AStar_vs_Dijkstra}, Dijkstrov algoritmus sa rozpína na všetky strany a algoritmus A* si ihneď vyberá vrchol najbližšie k cieľovému vrcholu. Zelenou farbou je znázornený počiatočný vrchol, modrou koncový. Žltou farbou sú vyobrazené algoritmom prejdené vrcholy, šedou tie naplánované a červenou nájdená cesta.\newline

\begin{figure}[H]
  \centering{\includegraphics{./images/3_AStar-Dijkstra.png}}
  \caption{Porovnanie A* (vľavo) a Dijkstrovho algoritmu (vpravo)}
  \label{AStar_vs_Dijkstra}
\end{figure}

Pri algoritme A* môže byť problémové určiť heuristickú funkciu. Zjavne, ak $h(x)=0$ pre všetky vrcholy, výber vrchola pre iteráciu algoritmu bude závisieť len od doposiaľ zistenej ceny cesty, a teda bude algoritmus prebiehať rovnako ako Dijkstrov algoritmus. Pre každú inú heuristickú funkciu, ktorá si ale zachová prípustnosť, je garantované zlepšenie v rýchlosti výpočtu algoritmu. Jednou z možností je zobrať Euklidovskú vzdialenosť (vzdialenosť vzdušnou čiarou) medzi uvažovaným a koncovým vrcholom. V našom prípade uvažujeme čas prepravenia sa medzi dvoma zastávkami, takže budeme musieť počítať i s rýchlosťou najrýchlejšieho spojenia, a tak zistili teoreticky najlepší čas cesty medzi nimi.

Časová zložitosť algoritmu je $O(|E|)$.
...
Uvedené vylepšenia sú iba názorným príkladom vybraných algoritmov z pestrej škály rôznych vylepšení a následných modifikácií základných algoritmov určených na vyhľadávanie v grafikonoch. Existujú teda i rýchlejšie spôsoby zistenia najlacnejších ciest v grafov, a to prevažna pomocou predpočítania si rôznych hodnôt a údajov. Dobrým príkladom môže byť takzvaný "Highway-node routing"\cite{schultes2007dynamic} alebo jeden z jeho špeciálnych prípadov "Contraction hierarchies"\cite{geisberger2008contraction}.


\subsection{Existujúce implementácie vyhľadávacích algoritmov}

https://github.com/graphhopper/graphhopper/tree/master/core/src/main/java/com/graphhopper/routing
https://stackoverflow.com/questions/14091279/algorithm-method-use-by-google-maps-for-finding-the-path-between-two-cities
https://www.microsoft.com/en-us/research/publication/round-based-public-transit-routing/
https://github.com/CanalTP/navitia/tree/dev/source/routing    - RAPTOR
https://www.google.sk/search?q=public+transport+search+open+source+algorithm&ei=KZ47W_yoKNLIwALs06jwDg&start=10&sa=N&biw=1536&bih=759
